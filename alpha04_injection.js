...
function a04UpdateJungle(dt,t){const mt=a04Time(t);for(const j of a04Jungle){if(!j.alive){if(mt+1e-7>=j.respawnAt){j.alive=true;j.hp=j.maxHp;j.group.position.copy(j.spawn);j.nextHit=mt+.75;j._a05PlayerHitUntil=0;j.respawnAt=Infinity;j.group.visible=riftVision.visible(j)}continue}const d=j.group.position.distanceTo(player.group.position);if(riftVision.canSee(j,player,7.2)){const v=player.group.position.clone().sub(j.group.position).setY(0);if(d>2){riftNavigation.steer(j,player.group.position,dt,BODY_RADIUS.jungle)}else if(mt>=j.nextHit){j.nextHit=mt+1.3;hurtPlayer(j.damage,j.type==='blue'||j.type==='red'?'magical':'physical')}}else{const home=j.spawn.clone().sub(j.group.position).setY(0);if(home.length()>1)riftNavigation.steer(j,j.spawn,dt,BODY_RADIUS.jungle,1.2)}}}
function a04UpdatePit(dt,t){const mt=a04Time(t);if(!a04Pit.alive){if((!a04PitSpawned&&mt>=210)||(a04PitRespawn&&mt>=a04PitRespawn)){a04PitRespawn=0;a04SpawnPit(true)}return}const d=a04Pit.group.position.distanceTo(player.group.position);if(riftVision.canSee(a04Pit,player,8)){const v=player.group.position.clone().sub(a04Pit.group.position).setY(0);if(d>2.8)riftNavigation.steer(a04Pit,player.group.position,dt,BODY_RADIUS.pitlord);else if(mt>=a04Pit.nextHit){a04Pit.nextHit=mt+1.45;hurtPlayer(a04Pit.damage,'magical')}}else if(groundDistanceSq(a04Pit.group.position,a04Pit.spawn)>.1)riftNavigation.steer(a04Pit,a04Pit.spawn,dt,BODY_RADIUS.pitlord);}
