...
export async function gameHarness({enter=true}={}){
  ...
  window.HTMLCanvasElement.prototype.getContext=function(){return new Proxy({},{get:(o,k)=>o[k]??(()=>{})});};
  ...
  const context=vm.createContext({THREE:{...T,WebGLRenderer:Renderer},...progression,...rules,createBattlefield,installRiftArt,createCombatVfx,createLobby,createMatchFlow,
  ...
  return {window,document:window.document,run,async dispose(){run('riftVfx.dispose();riftLobby.dispose()');await window.happyDOM.abort();}};
}
