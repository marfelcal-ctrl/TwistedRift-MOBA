...
Phone testing showed that the hero still looked too small, fog ignored walls, trees blocked walking, and local bush concealment was unclear. This update addresses those four issues.

- The battle camera now uses **26° yaw, 49° pitch and 9.5 orthographic half-height**. Hero art scales to `.68 / .86 / .68` on X/Y/Z for both RAMZX and the Sentinel. The idle RAMZX mesh projects to about **59 pixels tall on an 844 × 390 viewport**, compared with 37 pixels in Alpha 0.8.1. Portrait preserves the same pixel scale. Body radius, movement speed, reach and skill ranges retain their gameplay values.
- **Cliffs, ruins, lane dividers and Pitlord pillars block sight.** Exact segment tests use their rendered placement footprints. Living allies can reveal an area from the other side; camera panning cannot. The same rule governs units, target selection, health bars, minimap dots, combat reveals and effects. The fog texture now follows wall occlusion instead of painting unrestricted circles through terrain.
- **Decorative pine trees are passable.** Stone terrain and bridge rails retain movement collision. Rails are low enough to see over, and bridge decks remain walkable at their actual height. Trees do not grant concealment.
- **Tall grass hides heroes from observers outside the pocket.** An enemy entering that pocket can see its occupants if the sight line is clear. Nearby combat exposes a hero for two seconds, but never grants sight through a wall or beyond the observer's range. The local player remains controllable as a faded silhouette, with a **HIDDEN / REVEALED** badge. The Sentinel, jungle monsters and Pitlord respect sight when acquiring the player.
- Fog masks for stationary sources are cached. Moving sources check nearby terrain buckets at 10 Hz; combat visibility updates synchronously. The local silhouette uses coverage fading to preserve opaque depth ordering, and its material changes are isolated from other units and lobby models.

Vision radii in world units remain: hero 12, minion 7, tower 9.5, core 11. The shared 128 × 128 fog texture also supplies the minimap mask. Known terrain remains dimly visible; static camp/arena landmarks do not report whether a hidden monster is alive. Enemy units, their health bars and attached Deadline markers disappear when vision is lost. Aimed Sever can strike concealed enemies; basic attacks and Deadline cannot acquire them.

All **14 jungle camps first spawn at 0:24 of match time**. Lobby, ready check and loading time do not count. Their **45-second respawn after defeat** is separate from the initial spawn. Pitlord keeps its 3:30 first spawn, 180-second respawn and 90-second siege buff. Two lanes, wave pacing, skill/stat choices and the existing Twisted Rift combat rules are retained.

Movement collision uses ground-plane body clearance; weapon tips, capes and animated extremities are not separate physics shapes. Heroes, enemy AI, minions, jungle monsters and Pitlord use swept movement to prevent tunneling. AI routes around barriers through the defined jungle entrances. Repeated walls, trees and grass use instanced geometry.
...
The camera shows a consistent battlefield area rather than enlarging nearby units with perspective. Blue is bottom-left and red is top-right. Keyboard, joystick, drag aiming and camera panning use the same axes. Alpha 0.8.2 uses camera half-height 9.5 in landscape; portrait preserves 19 units of horizontal coverage. The camera and hero model adjustments are described above. Movement, aiming and panning use the inverse camera projection in both orientations.
...
Run `npm ci` followed by `npm test` (44 tests; Node 20.19+). Coverage includes progression boundaries, damage mitigation, shared clock behavior at 20/30/60/120 FPS, pause/resume, path corners, targeting, camera projection, automatic resolution behavior, model geometry/joint preservation, and VFX callbacks/cleanup.
...
Additional regression checks cover thin and rotated sight blockers, clear openings, wall occlusion in models/minimap/targeting/VFX, stationary fog caching and invalidation, local bush concealment and enemy pursuit, combat exposure expiry, isolated silhouette/fog shader composition, and actual hero-mesh projection at 844 × 390 and 390 × 844. Existing checks also cover the exact 0:24 jungle spawn, dash collision, AI detours, all camp routes, clear lane formations, passable pine trunks, bridge rails and deck traversal. These are simulation/CPU and DOM checks, not measured GPU FPS. The available cloud browser reports WebGL disabled and blocks the local HTML preview route, so GPU shader execution, final camera appearance and responsive touch layout have not been verified there. The new lobby/versus GPU portrait pass is also unverified on a real GPU. Review the visual flow and device FPS before merging. Existing models remain procedural stylized approximations, not photorealistic replicas.
