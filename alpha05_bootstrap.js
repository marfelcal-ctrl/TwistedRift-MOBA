...
try{
  const [baseRes,p04Res,p05Res,p06Res,p07Res,p08Res]=await Promise.all([
    fetch('./game.js?base=alpha08'),
    fetch('./alpha04_injection.js?patch=alpha08'),
    fetch('./alpha05_injection.js?patch=alpha08'),
    fetch('./alpha06_injection.js?patch=alpha08'),
    fetch('./alpha07_injection.js?patch=alpha08'),
    fetch('./alpha08_injection.js?patch=alpha08')
  ]);
  ...
  const artUrl=new URL('./art/game-art.mjs?version=alpha08',location.href).href;
  source=`import {installRiftArt} from ${JSON.stringify(artUrl)};\n`+source;
  source=`import {MATCH,mapCoordinate,mapPoints,groundDistanceSq,minimapProject,minimapUnproject,cameraBounds,createMatchClock,waveFormation,moveAlongPath,UnitGrid} from ${JSON.stringify(new URL('./match-rules.mjs',location.href).href)};\n`+source;
  ...
  for(const [name,path]of [['createLobby','lobby'],['createMatchFlow','match-flow']])source=`import {${name}} from ${JSON.stringify(new URL('./'+path+'.mjs?version=alpha08',location.href).href)};\n`+source;
  for(const [name,path]of [['createBattlefield','battlefield'],['createCombatVfx','combat-vfx'],['installGraphics','graphics']]){
    source=`import {${name}} from ${JSON.stringify(new URL('./art/'+path+'.mjs?version=alpha08',location.href).href)};\n`+source;
  }
  ...
  if(!source.includes(anchor))throw new Error('Alpha 0.8 could not find the Alpha 0.3 injection anchor.');
  source=source.replace(anchor,`${p04}\n${p05}\n${p06}\n${p07}\n${p08}\n${anchor}`);
  source=source.replace("ui.loadingText.textContent='The Twisted Rift battlefield is ready.';","ui.loadingText.textContent='Alpha 0.8 is ready. The gates are open.';");
  ...
  console.error(err);
  if(loadingText)loadingText.textContent=`Alpha 0.8 failed to load: ${err.message}`;
}
